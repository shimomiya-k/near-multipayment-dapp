...
describe("Swap Contract", function () {
  ...
  describe("Deployment", function () {
    ...
    it("ERC20 token is minted from smart contract", async function () {
      ...
      expect(balanceOfDai.toString()).to.eq("1000000000000000000000000"); // => Add
    });
    ...
    it("Get value between DAI and ETH", async function () {
      ...
      const result = value / parseInt(ethers.utils.parseEther("1").toString()); // => Add
      console.log(`value of ETH/DAI is ${result}`); // => Add
      expect(result).to.eq(0.1); // => Add
    });
    ...
    it("swap function", async function () {
      ...
      expect(ethAmountBefore.toString()).to.eq("0"); // => Add
      ...
      expect(ethAmountAfter).to.eq("0.1"); // => Add
    });
  });
});
